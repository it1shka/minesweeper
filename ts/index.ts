...
function createField(root: HTMLElement, size: number, dispatcher: Dispatcher) {
...
function cleanupField(root: HTMLElement) {
  while(root.firstChild) {
    root.removeChild(root.lastChild!)
  }
}

function play(boardSize: number, bombPercentage: number) {
...
function main() {
  const startForm = document.getElementById('start-game-form')! as HTMLFormElement
